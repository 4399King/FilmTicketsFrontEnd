<template>
    ...
    <import src="../../../templates/nothing/nothing.wxml" /> -->
		...
		<view class="container" :style="{ position: isShow ? 'fixed' : '' }">
          ...
          ></select-time> -->
					<choose-date :start-time="showTime" @changeTime="changeTime"></choose-date>
				</view>

				<view>
					<filter-bar :city-cinema-info="cityCinemaInfo" @cinemaList="getCinemas" @change="changeCondition"
						@toggleShow="toggleShow" :hidden="!isShow" :params="params"></filter-bar>
				</view>
			</view>
			<view class="main-content">
				<view class="cinema-list" v-if="cinemas.length">
					<!-- <view class="cinema-list" v-if="cinemas.length"> -->
					<cinema-list-item :movieId="movieId" :cinemas="cinemas" :day="params.day" @navTo="navTo"></cinema-list-item>
	...
	export default {
		...
		methods: {
			initPage({ allInfo }) {
				...
				console.log(allInfo, '???????')
				const { movieId, movieName, showTime } = JSON.parse(allInfo)
				this.movieId = movieId
				this.showTime = showTime //影片上映日期

				wx.setNavigationBarTitle({ title: movieName })
				this.params = {
					...this.params,
					movieId: this.movieId
				}
				console.log('options:', allInfo)
